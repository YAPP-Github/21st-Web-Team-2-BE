...
internal class AuthControllerTest : ApiControllerTest(uri = "/api/v1/auth") {
    ...
    @Test
    fun `회원가입`() {
        val signUpDto = SignUpDto("leah", "developer", 1)
        `when`(signService.signup("token", signUpDto)).thenReturn(jwtTokens)
    ...
    @Test
    fun `기가입자가 아닌 경우 로그인`() {
        `when`(signService.signIn("code")).thenReturn(SignIn(isMember = false, JwtTokens()))
    ...
    @Test
    fun `기가입자인 경우 로그인`() {
        `when`(signService.signIn("code")).thenReturn(SignIn(isMember = true, jwtTokens))
